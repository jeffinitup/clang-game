...
signal music_started( mod_name : String )
...
class GodotModPlayerEnvelope:
	...
	func update( ) -> void:
		...
		for i in Vector2i( 1, self.source.point_count ):
...
func _ready( ):
	...
	if AudioServer.get_bus_index( self.mod_master_bus_name ) == -1:
		...
		for i in range( 32 ):
			...
			for k in ( AudioServer.get_bus_count( ) ):
				if AudioServer.get_bus_name( k ) == self.mod_channel_bus_name % i:
					mod_channel_bus_idx = k
					break

			var cae: = GodotModPlayerChannelAudioEffect.new( )
			for k in ( AudioServer.get_bus_effect_count( mod_channel_bus_idx ) ):
...
func _prepare_to_play( ) -> void:
	...
	self.channel_status = []
	for i in ( self.mod_data.channel_count ):
...
func play( from_position:float = 0.0 ):
	...
	self.music_started.emit(self.mod_data.module_name)
